...
function search(keywords, options, callback){
    ...
    options.fuzzy = (options.fuzzy==undefined)?0:options.fuzzy;
    options.__fuzzy_retried = (options.__fuzzy_retried==undefined)?0:options.__fuzzy_retried;
    ...
    var preparams = function(kws){
        var params = [(KW+kws.join('+'))];
        for(var i=0;i<kws.length;i++)
        {
            params.push(KW+kws[i]);
        }
        return params;
    }
    ...
    client.exists(KW+newkey,function(err,reply){
        ...
        {
            // Not in cache
            client.sinterstore(preparams(keywords),function(err,count){

                // Fuzzy Query
                if(!err&&count=='0'&&keywords.length>=2&&options.fuzzy>0&&options.__fuzzy_retried<options.fuzzy){

                    /*
                    // Plan A: Pop & Shift ...
                    (options.__fuzzy_retried%3!=2)?keywords.pop():keywords.shift();

                    options.__fuzzy_retried++;
                    ...
                    */
                    ...
                    // Plan B
                    var last = keywords.pop();
                    options.__fuzzy_retried++;
                    client.sinterstore(preparams(keywords),function(err,rcount){
                        ...
                        if(rcount=='0')
                        {
                            ...
                            keywords.push(last);
                            keywords.shift();
                            client.sinterstore(preparams(keywords),function(err,lcount){
